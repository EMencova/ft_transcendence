...
export function TournamentView(push = true) {
    ...
    const container = createElement('div', { className: 'container mx-auto p-8' });
...
async function loadTournaments() {
    try {
        ...
        const table = createElement('table', { className: 'min-w-full border-collapse' });
        const thead = createElement('thead');
        const tbody = createElement('tbody');
        
        // Table header
        const headerRow = createElement('tr');
        ['ID', 'Name', 'Start Date', 'Status', 'Winner', 'Actions'].forEach(text => {
...
function showCreateTournamentForm() {
    ...
    nameField.appendChild(createElement('label', { 
    ...
    sizeField.appendChild(createElement('label', { 
        className: 'block mb-2', 
    ...
    playersField.appendChild(createElement('label', { 
        className: 'block mb-2', 
    ...
    try {
        ...
        players.forEach((player: any) => {
            const playerCard = createElement('div', { 
                className: 'border rounded p-4 cursor-pointer bg-gray-800 hover:bg-gray-700',
            ...
            // Player name
            playerCard.appendChild(createElement('p', {
            ...
            const indicator = createElement('div', {
                className: 'absolute top-2 right-2 w-6 h-6 rounded-full bg-gray-400 hidden'
            });
...
async function createTournament() {
    ...
    const sizeInput = document.querySelector('input[name="tournament-size"]:checked') as HTMLInputElement;
    ...
    const selectedPlayers = Array.from(document.querySelectorAll('#players-container > div.bg-gray-700, #players-container > div.bg-gray-500'));
    const playerIds = selectedPlayers.map(el => el.getAttribute('data-player-id'));
...
async function loadTournamentDetails(tournamentId: number) {
    try {
        ...
        infoContainer.innerHTML = `
            ...
            <p><strong>Start Date:</strong> ${new Date(data.tournament.start_date).toLocaleString()}</p>
            ${data.tournament.winner_id ? `
                <p><strong>Winner:</strong> ${players.find((p: any) => p.id === data.tournament.winner_id)?.username || 'Unknown'}</p>
            ` : ''}
        `;
        ...
        bracketContainer.innerHTML = 'Welcome to the Tournament Bracket!<br>Loading matches...';
        ...
        Object.keys(rounds)
            ...
            .forEach(roundNum => {
                ...
                roundDiv.appendChild(createElement('h3', {
        ...
        const bracketContainer = document.getElementById('tournament-bracket');
        if (bracketContainer) {
            bracketContainer.innerHTML = `<p class="text-red-500">Error loading tournament: ${error instanceof Error ? error.message : 'Unknown error'}</p>`;
        }
    }
}
...
function createMatchCard(match: any, players: any[]) {
    ...
    if (player1) {
        player1Div.appendChild(createElement('img', {
            className: 'w-8 h-8 rounded-full mr-2',
            src: player1.avatar || '/avatar.png',
            alt: player1.username
        }));
        player1Div.appendChild(createElement('span', {
    ...
    // VS separator
    matchCard.appendChild(createElement('div', {
    ...
    if (player2) {
        player2Div.appendChild(createElement('img', {
            className: 'w-8 h-8 rounded-full mr-2',
            src: player2.avatar || '/avatar.png',
            alt: player2.username
        }));
        player2Div.appendChild(createElement('span', {
    ...
    if (match.status === 'scheduled' && player1 && player2) {
        const startBtn = createElement('button', {
            className: 'mt-3 w-full py-1 bg-orange-500 text-white rounded hover:bg-orange-600',
            textContent: 'Start Match'
        });
        startBtn.addEventListener('click', () => startTournamentMatch(match.id));
        ...
        resultForm.appendChild(select);
        
        const recordBtn = createElement('button', {
            className: 'w-full py-1 bg-green-500 text-white rounded hover:bg-green-600',
            textContent: 'Record Result'
        });
        recordBtn.addEventListener('click', () => recordMatchResult(match.id, select.value));
        resultForm.appendChild(recordBtn);
...
async function startTournamentMatch(matchId: number) {
    try {
        const response = await fetch(`/api/tournaments/matches/${matchId}/start`, {
            method: 'POST'
        });
...
async function recordMatchResult(matchId: number, winnerId: string) {
    try {
        ...
        if (!response.ok) throw new Error('Failed to record result');
