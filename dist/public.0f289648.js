...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (canvas && context) {
        ...
        startBtn.addEventListener('click', ()=>{
            ...
            gameRunning = true;
            (0, _game.startGame)(canvas, context, gameType, difficulty);
        });
    ...
    } else console.error("Canvas not supported or context could not be retrieved");
});

},{"./game":"7eSYL"}],"7eSYL":[function(require,module,exports,__globalThis) {
...
let gamePaused = true;
function startGame(canvas, ctx, gameType, difficulty) {
    const AI_DIFFICULTY = DIFFICULTY[difficulty];
    let state;
    if (gameType === "Pong1") state = {
        gameType: "Pong1",
        player1: {
            x: 0,
        ...
        player2: {
            ...
            calculateMove: (gameState)=>{
                ...
                if (ai.y + ai.height / 2 < targetY) return {
                    direction: 'down'
                };
                else if (ai.y + ai.height / 2 > targetY) return {
                    direction: 'up'
                };
                return {
                    direction: 'none'
    ...
    window.addEventListener('pauseStateChanged', (e)=>{
    ...
    function movePaddles() {
        ...
        if (state.gameType === "Pong1") {
            const move = state.player2.calculateMove(state);
    ...
    function detectCollision(paddle, ball) {
        return ball.x - ball.radius < paddle.x + paddle.width && ball.x + ball.radius > paddle.x && ball.y - ball.radius < paddle.y + paddle.height && ball.y + ball.radius > paddle.y;
    }
    function update() {
        movePaddles();
        ...
        if (detectCollision(state.player1, state.ball)) state.ball.velocityX = Math.abs(state.ball.velocityX);
        else if (detectCollision(state.player2, state.ball)) state.ball.velocityX = -Math.abs(state.ball.velocityX);
        if (state.ball.x < 0) {
            state.score2++;
            resetBall();
...
},{}]},["elbaT","gNc1f"], "gNc1f", "parcelRequire1357", {})

//# sourceMappingURL=public.0f289648.js.map
